...
class logistic_score_card(object):
    ...
    def plot_iv_graph(self, data, path):
        logging.info('....IV图绘制进行中.....')
        for col, cut_points in tqdm(self.cut_points_result):
            data_col = data[[col]]
            bad_rate_result = []     #绘制IV图时的Y轴
            index_list = []          #存储绘制IV图时的X轴
            #删除cut_points中的'null'
            for i in range(len(cut_points)):
                if cut_points[i] == 'null':
                    del cut_points[i]
            #cut_points增加最大值
            col_max = data_col[col].max()
            cut_points.append(col_max)
            cut_points = list(set(cut_points))
            #cut_points排序
            cut_points = sorted(cut_points)
            #print(cut_points,col)
            #变量中的空值单独作为一个分箱
            if len(data[data[col].isnull()]) > 0:
                null_size = np.sum(data[col].isnull())
                null_bad_size = np.sum((data[col].isnull()) & (data['y'] == 1))
                null_bad_rate = round((null_bad_size / null_size),3)
                index_list.append('Nan')
                bad_rate_result.append([null_size,null_bad_rate])
            #非空值分箱IV值统计
            for j in range(len(cut_points)):
                if j==0:       #第一个切分点
                    num_size = len(data[data[col]<=cut_points[j]])
                    bad_size = len(data[(data[col]<=cut_points[j]) & (data['y']==1)])
                    bad_rate = round((bad_size / num_size),3)
                    index_list.append(str(round(cut_points[j],3)))
                    bad_rate_result.append([num_size,bad_rate])
                else:
                    num_size = len(data[(data[col]>cut_points[j-1]) & (data[col]<=cut_points[j])])
                    bad_size = len(data[(data[col]>cut_points[j-1]) & (data[col]<=cut_points[j]) & (data['y']==1)])
                    #print(num_size,bad_size,j)
                    bad_rate = round((bad_size / num_size),3)
                    index_list.append(str(round(cut_points[j],3)))
                    bad_rate_result.append([num_size,bad_rate])
            graph_value = pd.DataFrame(bad_rate_result,index=index_list)
            graph_value.columns = ['num_size','bad_rate']
            #将cut_point列变成索引供绘图
            col_type_cut_iv = self.col_type_cut_iv
            params = pd.DataFrame(col_type_cut_iv).T  #行列变换
            params.columns = np.array(params.iloc[0:1]).tolist()
            iv = round(params['date_ratio_15'].values[2][0],3)
            #绘制每一个变量的IV图
            plt.figure()
            plt.title('%s - IV = %s' % (col,iv))
            plt.ylabel('样本数量')
            plt.xlabel('分组编号')
            graph_value['num_size'].plot(kind = 'bar')
            graph_value['bad_rate'].plot(color='r',secondary_y=True,style='-o',linewidth=2)
            plt.legend(loc='lower right')
            plt.savefig(path + '/{0} - {1}.png'''.format(col,iv))
            plt.show()